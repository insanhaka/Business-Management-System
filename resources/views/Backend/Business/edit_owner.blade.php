...
<div class="container-fluid" style="margin-top: 3%; margin-bottom: 6%;">
    ...
    <div class="card">
        <div class="card-header">
          <h2 class="text-primary">Edit Business Owner Data</h2>
        </div>
        <div class="card-body">
            <form method="POST" action="/dapur/business/owner/update/{!!$data->id!!}" enctype="multipart/form-data">
                {{ csrf_field() }}
                <div class="container" style="margin-top: -10px;">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="name" name="name" placeholder="Business owner name" value="{!!$data->name!!}">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="exampleFormControlInput1">NIK</label>
                                <input type="text" class="form-control" id="nik" name="nik" placeholder="NIK" maxlength="18" value="{!!$data->nik!!}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="exampleInput">Upload Foto Profil</label>
                                <div class="tower-file">
                                    <input type="file" name="photo" id="demoInput6" />
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="ktp-address" name="ktp_address" placeholder="Details owner address" value="{!!$data->ktp_address!!}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="exampleFormControlInput1">Details Alamat</label>
                                <input type="text" class="form-control" id="domisili-address" name="domisili_address" placeholder="Details owner address" value="{!!$data->domisili_address!!}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
<script>
    ...
    $(document).ready(function() {
        ...
        let prov = $('#ktp-province');
            prov.empty();
            prov.append($('<option></option>').attr('value', '{!! $data->ktp_loc_province !!}').text('{!! $data->ktp_loc_province !!}'));
            prov.prop('selectedIndex', 0);
        let reg = $('#ktp-regency');
            reg.empty();
            reg.append($('<option></option>').attr('value', '{!! $data->ktp_loc_regency !!}').text('{!! $data->ktp_loc_regency !!}'));
            reg.prop('selectedIndex', 0);
        let dist = $('#ktp-district');
            dist.empty();
            dist.append($('<option></option>').attr('value', '{!! $data->ktp_loc_district !!}').text('{!! $data->ktp_loc_district !!}'));
            dist.prop('selectedIndex', 0);
        let vill = $('#ktp-village');
            vill.empty();
            vill.append($('<option></option>').attr('value', '{!! $data->ktp_loc_village !!}').text('{!! $data->ktp_loc_village !!}'));
...
<script>
    ...
    $(document).ready(function() {
        ...
        let prov = $('#domisili-province');
            prov.empty();
            prov.append($('<option></option>').attr('value', '{!! $data->domisili_loc_province !!}').text('{!! $data->domisili_loc_province !!}'));
            prov.prop('selectedIndex', 0);
        let reg = $('#domisili-regency');
            reg.empty();
            reg.append($('<option></option>').attr('value', '{!! $data->domisili_loc_regency !!}').text('{!! $data->domisili_loc_regency !!}'));
            reg.prop('selectedIndex', 0);
        let dist = $('#domisili-district');
            dist.empty();
            dist.append($('<option></option>').attr('value', '{!! $data->domisili_loc_district !!}').text('{!! $data->domisili_loc_district !!}'));
            dist.prop('selectedIndex', 0);
        let vill = $('#domisili-village');
            vill.empty();
            vill.append($('<option></option>').attr('value', '{!! $data->domisili_loc_village !!}').text('{!! $data->domisili_loc_village !!}'));
            vill.prop('selectedIndex', 0);
