...
<div class="container-fluid" style="margin-top: 3%; margin-bottom: 6%;">
    ...
    <div class="card">
        <div class="card-header">
          <h2 class="text-primary">Edit Data Kelompok / Paguyuban</h2>
        </div>
        <div class="card-body">
            <form method="POST" action="/dapur/community/update/{!! $data->id!!}">
                {{ csrf_field() }}
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="name" name="name" placeholder="Nama Kelompok" value="{!! $data->name!!}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="chairman_name" name="chairman_name" placeholder="Nama Ketua Kelompok" value="{!! $data->chairman_name!!}">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="exampleFormControlInput1">NIK</label>
                                <input type="text" class="form-control" id="chairman_nik" name="chairman_nik" placeholder="NIK Ketua" maxlength="18" value="{!! $data->chairman_nik!!}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="ktp-address" name="office_address" placeholder="Alamat kesekretariatan lengkap" value="{!! $data->office_address !!}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
<script>
    ...
    $(document).ready(function() {
        ...
        let prov = $('#ktp-province');
            prov.empty();
            prov.append('<option value="{!! $data->office_loc_province!!}">{!! $data->office_loc_province!!}</option>');
            prov.prop('selectedIndex', 0);
        let reg = $('#ktp-regency');
            reg.empty();
            reg.append('<option value="{!! $data->office_loc_regency!!}">{!! $data->office_loc_regency!!}</option>');
            reg.prop('selectedIndex', 0);
        let dist = $('#ktp-district');
            dist.empty();
            dist.append('<option value="{!! $data->office_loc_district!!}">{!! $data->office_loc_district!!}</option>');
            dist.prop('selectedIndex', 0);
        let vill = $('#ktp-village');
            vill.empty();
            vill.append('<option value="{!! $data->office_loc_village!!}">{!! $data->office_loc_village!!}</option>');
